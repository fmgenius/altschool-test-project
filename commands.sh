...
vagrant ssh master <<EOF
    ...
    sudo cat /home/vagrant/.ssh/id_rsa.pub | sshpass -p "vagrant" ssh -o StrictHostKeyChecking=no vagrant@192.168.33.58 'mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys'
    sudo cat ~/altschoolkey | sshpass -p "vagrant" ssh vagrant@192.168.33.58 'mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys'
    ...
    sshpass -p "femi" sudo -u altschool scp -r /mnt/* altschool@192.168.33.57:/home/vagrant/mnt
